...
export default function EditChitFundScreen() {
  ...
  // Load existing chit fund data
  useEffect(() => {
    const loadChitFund = async () => {
      try {
        ...
        setName(chitFundData.name);
        setTotalMembers(chitFundData.total_members.toString());
        setMonthlyAmount(chitFundData.monthly_amount.toString());
        setDurationMonths(chitFundData.duration_months.toString());
        setStartDate(chitFundData.start_date);
        setForemanCommission(chitFundData.foreman_commission.toString());
        setMyTurnMonth(chitFundData.my_turn_month?.toString() || '');
        setNotes(chitFundData.notes || '');
      } catch (error) {
        console.error('Failed to load chit fund:', error);
  ...
  const handleSave = async () => {
    ...
    try {
      await updateChitFund(chitFundId, {
        name: name.trim(),
      ...
      setSnackbarMessage('Chit fund updated successfully');
      ...
      console.error('Failed to update chit fund:', error);
      Alert.alert('Error', 'Failed to update chit fund. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = () => {
    Alert.alert(
      'Delete Chit Fund',
      'This will permanently delete this chit fund and all its records. This cannot be undone.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deleteChitFund(chitFundId);
              setSnackbarMessage('Chit fund deleted successfully');
              setSnackbarVisible(true);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Edit Chit Fund</Text>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        {/* Status Info */}
        {chitFund && chitFund.status === 'completed' && (
          <View style={styles.section}>
            <View style={styles.statusInfo}>
              <Text style={styles.statusTitle}>Status: Completed</Text>
              <Text style={styles.statusDescription}>
                This chit fund has been completed. You can still edit details but the status remains completed.
              </Text>
            </View>
          </View>
        )}
