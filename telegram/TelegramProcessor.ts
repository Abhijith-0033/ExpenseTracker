...
export async function processPendingTransaction(transaction: any): Promise<{
  ...
  try {
    ...
    // Categories are stored as JSON in accounts table, not a separate table.
    // The transaction.category is already the name string from the bot.
    // We just need to confirm it's a valid category; if not, fall back to first expense category.
    const categories = await getCategories();
    const expenseCategories = categories.filter(c => c.name !== 'Income');

    let resolvedCategory = transaction.category || 'Other';
    const categoryExists = categories.some(
      c => c.name.toLowerCase() === resolvedCategory.toLowerCase()
    );
    if (!categoryExists) {
      // Fall back to first expense category or 'Other'
      resolvedCategory = expenseCategories[0]?.name || 'Other';
    }
    ...
    if (defaultAccountId) {
      const found = accounts.find(a => a.id.toString() === defaultAccountId);
      if (found) account = found;
    }

    // STEP 3 — Duplicate check
    // Look for an identical transaction within the last 5 minutes
    ...
    if (duplicate) {
      return { success: false, error: 'Duplicate transaction detected (same amount+category in last 5 min)' };
    }

    // STEP 4 — Insert transaction using the new bypass function
    const txId = await addTransactionDirect({
      amount: transaction.amount,
      category: resolvedCategory,
      subcategory: '',           // Telegram doesn't know subcategory
